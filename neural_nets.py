...
class AdaptUnet(nn.Module):
    ...
    def __init__(self, in_channels=3, out_channels=2,  alpha=0.1):
        ...
        self.classifier_side = nn.Sequential(
            nn.Dropout(),
            nn.Linear(1024, 4096),
    ...
    def forward(self, x, task='main'):
        ...
        if task == 'main':
            ...
            x = self.avgpool(bottleneck)
            #print(x.shape)
            x = torch.flatten(x, 1)
            x = ReverseLayerF.apply(x, self.alpha)
            #print(x.shape)
            x = self.classifier_side(x)
            discriminator_output = x
            return discriminator_output
        
        #return self.conv(dec1),  discriminator_output
